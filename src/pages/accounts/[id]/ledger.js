    ...
    return (
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden mb-6">
            <div className="text-center py-12">
                ...
                <h3 className="mt-2 text-xl font-semibold text-slate-900">No transactions found</h3>
                <p className="mt-1 text-slate-500">
                    There are no entries recorded for {accountName}.
                </p>
                <div className="mt-6">
                    <Link
                        href="/transactions/new" // Assuming you have a page to add new transactions
...
export default function AccountLedgerPage({ initialData, error }) {
        ...
        return (
            <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
                <div className="bg-white p-8 rounded-2xl shadow-md max-w-md w-full text-center border border-red-200">
                    <h1 className="text-2xl font-bold text-red-600 mb-4">Error Loading Ledger</h1>
                    <p className="text-slate-700 mb-4">{fetchError}</p>
                    <Link href="/accounts" className="text-blue-600 hover:underline font-medium">
                        Go back to Accounts
                    </Link>
                </div>
            </div>
        ...
        return (
            <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
                <div className="bg-white p-8 rounded-2xl shadow-md max-w-md w-full text-center border border-slate-200">
                    <h1 className="text-2xl font-bold text-slate-800 mb-4">Ledger Not Found</h1>
                    <p className="text-slate-700 mb-4">The ledger for account ID "{id}" could not be found.</p>
                    <Link href="/accounts" className="text-blue-600 hover:underline font-medium">
                        Go back to Accounts
                    </Link>
                </div>
    ...
    const { accountName, accountType, openingBalance, ledgerEntries, accountCode } = ledgerData;
            ...
            <div className="max-w-7xl mx-auto">
                <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 mb-6">
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        <div>
                            <h1 className="text-3xl font-bold text-slate-900 mb-2">Ledger for {accountName}</h1>
                            <p className="text-slate-600">Detailed transaction history for this account</p>
                        </div>
                        <Link
                            href="/accounts"
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                    <div className="p-5 bg-white rounded-2xl shadow-sm border border-slate-200">
                        ...
                        <p className="text-xl font-semibold text-slate-800 mt-1">#{accountCode || id}</p>
                    </div>
                    <div className="p-5 bg-white rounded-2xl shadow-sm border border-slate-200">
                        <p className="text-sm font-medium text-slate-600">Account Type</p>
                        <p className="text-xl font-semibold text-slate-800 mt-1">{accountType}</p>
                    </div>
                    <div className="p-5 bg-white rounded-2xl shadow-sm border border-slate-200">
                ...
                <div className="flex justify-end gap-3 mb-6">
                    <button
                        ...
                        className="inline-flex items-center justify-center px-5 py-2.5 bg-gradient-to-r from-red-500 to-red-600 text-white font-semibold rounded-lg hover:from-red-500 hover:to-red-700 transition-all duration-200 shadow-md hover:shadow-lg transform hover:-translate-y-0.5"
                    >
                    ...
                    <button
                        ...
                        className="inline-flex items-center justify-center px-5 py-2.5 bg-gradient-to-r from-green-500 to-green-600 text-white font-semibold rounded-lg hover:from-green-500 hover:to-green-700 transition-all duration-200 shadow-md hover:shadow-lg transform hover:-translate-y-0.5"
                    >
                        <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <table className="min-w-full divide-y divide-slate-200">
                                <thead className="bg-gradient-to-r from-slate-50 to-slate-100">
                                    <tr>
                                        ...
                                        <th className="px-6 py-4 text-right text-xs font-semibold text-slate-700 uppercase tracking-wider">Debit (₹)</th>
                                        <th className="px-6 py-4 text-right text-xs font-semibold text-slate-700 uppercase tracking-wider">Credit (₹)</th>
                                ...
                                <tbody className="bg-white divide-y divide-slate-200">
                                    {ledgerEntries.map((entry, index) => (
                                        <tr key={entry.id} className={`hover:bg-slate-50 transition-colors duration-150 ${index % 2 === 0 ? 'bg-white' : 'bg-slate-25'}`}>
    ...
    try {
        ...
        if (!res.ok) {
            return {
                props: {
                    initialData: null,
                    error: data.message || `Failed to fetch ledger data for account ID: ${id}`,
                },
            };
        }
