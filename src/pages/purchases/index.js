...
export default function PurchasesList({ initialPurchases }) {
  ...
  const filteredPurchases = purchases.filter(purchase => {
    const matchesSupplier = filterSupplier === '' || 
      purchase.supplier.name.toLowerCase().includes(filterSupplier.toLowerCase());
  ...
  const totalPaid = filteredPurchases.reduce((sum, purchase) => sum + purchase.paidAmount, 0);
  const totalBalance = filteredPurchases.reduce((sum, purchase) => sum + purchase.balance, 0);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 mb-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div>
                <h1 className="text-3xl font-bold text-slate-900 mb-2">Purchases</h1>
                <p className="text-slate-600">Track and manage all your purchase transactions</p>
              </div>
              <Link
                href="/purchases/add"
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 mb-6">
            <div className="mb-4">
              <h3 className="text-lg font-semibold text-slate-900 mb-2">Filter Purchases</h3>
              <p className="text-sm text-slate-600">Use filters to find specific purchases</p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
            <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-2xl p-6 border border-blue-200">
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  <h3 className="text-sm font-medium text-blue-900">Total Purchases</h3>
            ...
            <div className="bg-gradient-to-br from-purple-50 to-purple-100 rounded-2xl p-6 border border-purple-200">
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  <h3 className="text-sm font-medium text-purple-900">Total Amount</h3>
                  ...
                  <tbody className="bg-white divide-y divide-slate-200">
                    {filteredPurchases.map((purchase, index) => (
                      <tr key={purchase.id} className={`hover:bg-slate-50 transition-colors duration-150 ${index % 2 === 0 ? 'bg-white' : 'bg-slate-25'}`}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm font-semibold text-slate-900">{purchase.supplier.name}</div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm font-semibold text-slate-900">₹{purchase.totalAmount.toFixed(2)}</div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm text-green-600 font-medium">₹{purchase.paidAmount.toFixed(2)}</div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold ${
                            ...
                            {purchase.balance > 0 && <span className="ml-1 text-xs">(Due)</span>}
                          </span>
                        </td>
