// components/suppliers/SuppliersTable.js

import Link from 'next/link';
import { useState } from 'react';

export default function SuppliersTable({ suppliers, onDeleteClick }) {
    ...
    return (
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden mb-6 text-center py-12">
        ...
        <p className="mt-1 text-sm text-slate-500">
          Try adjusting your search terms or get started by adding your first supplier.
        </p>
        <div className="mt-6">
          <Link
            href="/suppliers/add"
  ...
  const toggleDropdown = (supplierId) => {
    setOpenDropdown(openDropdown === supplierId ? null : supplierId);
  };
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden mb-6">
        ...
        <div className="block md:hidden">
          {suppliers.map((supplier) => {
            ...
            return (
              <div key={supplier.id} className="border-b border-slate-200 last:border-b-0">
                <div className="p-4">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex-1 min-w-0">
                      ...
                      <h3 className="text-lg font-semibold text-slate-900 truncate">{supplier.name}</h3>
                    </div>
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="text-slate-900">
                        {supplier.phone || <span className="text-slate-400 italic">No phone</span>}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-500">Address:</span>
                      <span className="text-slate-900 text-right max-w-48 truncate">
                        {supplier.address || <span className="text-slate-400 italic">No address</span>}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-500">Total Purchases:</span>
                      <span className="text-slate-900 font-medium">₹{totalPurchases.toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between">
