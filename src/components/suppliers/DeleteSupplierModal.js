// components/suppliers/DeleteSupplierModal.js
import React from 'react';

export default function DeleteSupplierModal({ isOpen, supplier, isDeleting, onConfirm, onCancel }) {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-6">
        <div className="flex items-center mb-4">
          ...
          <div>
            <h3 className="text-lg font-semibold text-slate-900">Confirm Supplier Deletion</h3>
            <p className="text-slate-600">This action cannot be undone.</p>
          </div>
        </div>
        <p className="text-slate-700 mb-6">
          Are you sure you want to delete supplier "<span className="font-medium">{supplier?.name}</span>"?
          <br /><br />
